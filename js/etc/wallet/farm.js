    ...
    farmContract.myCrop.call(function (err, cropAddress) {
        ...
        crop.cropDividends.call(true, function (err, result) {
            if (!err) {
                ...
                myCropDividendsPoints = (web3.fromWei(myCropDividends) / buyPrice) console.log(myCropDividendsPoints)
                if (change) {
        ...
        cropAbi.at(cropAddress).transfer.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("<h3>Transfer " + amountToTransfer + " Point ke " + destination.substring(0, 7) + "...</h3>")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
