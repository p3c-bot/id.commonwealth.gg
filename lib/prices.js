...
function setMarketCap(idrPrice) {
  p3cContract.totalEthereumBalance.call(function (err, result) {
    if (!err) {
      ...
      $('#etcInContractUSDPrice').text('(Rp ' + numberWithCommas(Number((amount * idrPrice).toFixed(0)))+ ')')
...
function setSellPrice(idrPrice) {
  p3cContract.sellPrice(function (e, r) {
    ...
    $('#tokenUSDSellPrice').text('Rp ' + (sellPrice * idrPrice).toFixed(2))
  })
}
setSellPrice(0)
...
function setBuyPrice(idrPrice) {
  p3cContract.buyPrice(function (e, r) {
    ...
    $('#tokenUSDBuyPrice').text('Rp ' + (buyPrice * idrPrice).toFixed(2))
  })
}
setBuyPrice(0)
...
function setTokensPrice(idrPrice){
  ...
  $('#myTokensValue').text('Rp ' + numberWithCommas(value.toFixed(2)))
...
function setDividendsPrice(idrPrice){
  ...
  $('#myDividendsValue').text('Rp ' + value.toFixed(5))
...
$('#sellInput').on('input change', function () {
  ...
  if (value > 0) {
    ...
    $('#sellAmount').text("Rp " + sellAmountUSD + "/" + sellAmount + " ETC")
  } else {
    $('#sellAmount').hide()
  }
})
