...
function getMyCrop() {
    setTimeout(function () {
        ...
        if (web3.eth.accounts[0] === undefined) { 
            alertify.error('Tidak terhubung ke Commonwealth.')
        } else {
            alertify.success('Terhubung ke Commonwealth.')
        }
    }, 1000)
...
function buyFromCrop(amountToBuy, referrer) {
    ...
    p3cContract.buy.sendTransaction(
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                alertify.success(amountToBuy + " Menukar ETC. Menunggu Confirm Blockchain.")
            } else {
                console.log(error);
            }
        }
...
function sellFromCrop(amountToSell) {
    ...
    p3cContract.sell.sendTransaction(
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                alertify.success(amountToSell + " Poin Terjual. Menunggu Confirm Blockchain.")
                console.log(result);
            } else {
                console.log(error);
            }
...
function reinvestFromCrop(referrer) {
    p3cContract.reinvest.sendTransaction(
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                alertify.success("Reinvestasi Poin. Menunggu Confirm Blockchain.")
                console.log(result);
            } else {
                console.log(error);
            }
...
function withdrawFromCrop() {
    p3cContract.withdraw.sendTransaction({
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                alertify.success("Menarik dividen ke dompet ETC Anda.")
                console.log(result);
            } else {
                console.log(error);
            }
...
function transferFromCrop(destination, amountToTransfer) {
    ...
    p3cContract.transfer.sendTransaction(
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                alertify.success("Transfer " + amountToTransfer + " Poin ke " + destination.substring(0, 7) + "...")
                console.log(result);
            } else {
                console.log(error);
            }
