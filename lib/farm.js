    ...
    if (myCropAddress == 'null' || myCropAddress == null || myCropAddress == "0x") {
        farmContract.myCrop.call(function (err, result) {
            ...
            if (onboard == true && (result == '0x0000000000000000000000000000000000000000')) {
                setTimeout(function () {
                    alertify.confirm(
                        'Selamat datang di Commonwealth.gg!',
                        `
                        <h1 id="loginWarning" class="login-warning">Masuk ke dompet Saturnus, dan segarkan!</h1>
                        <p id="agreement" class="agreement">
                        Commonwealth adalah aplikasi Ethereum Classic. Membacanya di <a target="_blank" href="https://blockscout.com/etc/mainnet/address/0xde6fb6a5adbe6415cdaf143f8d90eb01883e42ac/contracts">Sini</a>. Dengan melanjutkan, Anda menerima bahwa kode akan berjalan seperti tertulis, tanpa jaminan keuntungan.
                         Untuk memulai, klik "Terima" dan selesaikan transaksi.
                        </p>
                        ...
                        function () {
                            ...
                            alertify.error('<h3>Lihat Mode.</h3>')
                    }).set({
                        labels: {
                            ok: 'Terima',
                            cancel: 'Batal'
                ...
                if (result !== '0x0000000000000000000000000000000000000000') {
                    ...
                    if (getETCWallet == false){
                        alertify.success('<h3>Aktifkan Mode ETC</h3>')
                        getETCWallet = true;
                    }
    ...
    farmContract.createCrop.sendTransaction(
        ...
        function (error, result) { //get callback from function which is your transaction key
            if (!error) {
                ...
                alertify.success("Selamat datang di Commonwealth! Harap tunggu 30 detik agar Crop Anda dibuat.")
                // playSound('register');
            } else {
                alertify.error("Akun baru ditolak. Lihat mode.")
            }
        })
}
        ...
        cropAbi.at(cropAddress).disable.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("Auto-Reinvest beralih ke " + !(state))
                    // playSound('register');
                    $('#reinvest').prop("disabled", true);
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).buy.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success('<h3>' + amountToBuy + " ETC dihabiskan. Menunggu Blockchain.</h3>")
                    // playSound('register');
                    $("#buy").transition({
                        animation: 'tada',
                        duration: '1s',
        ...
        cropAbi.at(cropAddress).sell.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success('<h3>' + amountToSell + " Poin Dijual. Dana dikirim ke dompet sebagai ETC.</h3>")
                    // playSound('register');
                    $('#sellAmount').hide();
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).reinvest.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("<h3>Saldo Hasil Dividen telah diinvestasi. Menunggu Blockchain.</h3>")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).withdraw.sendTransaction({
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("<h3>Menarik dividen ke dompet ETC Anda.</h3>")
                    console.log(result);
                    // playSound('register');
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).transfer.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("<h3>Transfer " + amountToTransfer + " Poin ke " + destination.substring(0, 7) + "...</h3>")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
